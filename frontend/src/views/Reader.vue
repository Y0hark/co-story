<template>
  <div class="min-h-screen bg-stone-50 pb-32" :class="fontFamily">
    <div class="max-w-3xl mx-auto px-4 md:px-6 pt-6 md:pt-12">
        ...
        <div v-else-if="!story || !chapters.length" class="text-center p-10 md:p-20 text-stone-500">
            ...
            <button class="mt-4 text-indigo-600 underline hover:text-indigo-700" @click="$router.push('/app/community')">Back to Community</button>
        </div>

        <div v-else>
            <!-- Navigation / Header -->
            <div class="mb-8 md:mb-12 flex items-center justify-between text-stone-500 text-sm relative">
                <button class="hover:text-stone-800 flex items-center gap-2 transition-colors p-2 -ml-2 rounded-lg active:bg-stone-100 hover-sketch" @click="$router.push('/app/community')">
                    <ArrowLeft class="w-5 h-5 md:w-4 md:h-4" /> <span class="hidden md:inline">Back</span>
                </button>
                <div class="flex items-center gap-2 md:gap-4">
                    <!-- Chapter Menu -->
                    <div class="relative">
                        <button @click="showChapterMenu = !showChapterMenu" class="hover:text-indigo-600 transition-colors flex items-center gap-2 p-2 rounded-lg active:bg-stone-100 hover-sketch" title="Table of Contents">
                        ...
                        <div v-if="showChapterMenu" class="absolute top-10 right-0 w-[calc(100vw-2rem)] md:w-72 bg-white shadow-xl rounded-xl border border-stone-100 z-50 py-2 max-h-[60vh] overflow-y-auto">
                            <button 
                                ...
                                :class="{'text-indigo-700 font-medium bg-indigo-50/50': idx === currentChapterIndex}"
                            >
                                <span class="truncate flex items-center gap-3">
                                    <Check v-if="chapter.is_read" class="w-4 h-4 text-indigo-500" />
                    ...
                    <!-- Settings -->
                    <div class="relative">
                         <button @click="showSettings = !showSettings" class="hover:text-stone-800 transition-colors p-2 rounded-lg active:bg-stone-100 hover-sketch" title="Settings">
                        ...
                        <div v-if="showSettings" class="absolute top-10 right-0 w-[calc(100vw-2rem)] md:w-72 bg-white shadow-xl rounded-xl border border-stone-100 z-50 p-4 space-y-4">
                            <div>
                                ...
                                <div class="flex bg-stone-100 rounded-lg p-1">
                                    <button @click="setFont('font-sans')" class="flex-1 py-2 md:py-1 text-sm rounded-md transition-all" :class="fontFamily === 'font-sans' ? 'bg-white shadow-sm text-indigo-700 font-medium' : 'text-stone-500 hover:text-stone-700'">Sans</button>
                                    <button @click="setFont('font-serif')" class="flex-1 py-2 md:py-1 text-sm rounded-md transition-all font-serif" :class="fontFamily === 'font-serif' ? 'bg-white shadow-sm text-indigo-700 font-medium' : 'text-stone-500 hover:text-stone-700'">Serif</button>
                                    <button @click="setFont('font-mono')" class="flex-1 py-2 md:py-1 text-sm rounded-md transition-all font-mono" :class="fontFamily === 'font-mono' ? 'bg-white shadow-sm text-indigo-700 font-medium' : 'text-stone-500 hover:text-stone-700'">Mono</button>
                                </div>
                            </div>
                            <div>
            ...
            <header class="text-center mb-10 md:mb-16 space-y-4">
                <div class="flex justify-center gap-2 mb-4">
                    <span v-if="story.mode || story.genre" class="inline-block px-3 py-1 rounded-full border border-indigo-200 bg-indigo-50 text-indigo-700 text-[10px] md:text-xs tracking-wider uppercase font-medium">
            ...
            <article class="prose prose-stone prose-lg md:prose-xl mx-auto leading-loose transition-all duration-300 px-1" :class="[fontSize, fontFamily === 'font-mono' ? 'max-w-4xl' : 'max-w-2xl']">
                <p 
                    ...
                    :class="{
                        'first-letter:text-4xl md:first-letter:text-5xl first-letter:float-left first-letter:mr-2 md:first-letter:mr-3 first-letter:text-indigo-700 first-letter:font-bold': index === 0
                    }"
                    v-html="paragraph"
                >
                </p>
            ...
            <div class="mt-12 md:mt-20 pt-8 md:pt-12 border-t border-stone-200 flex flex-col md:flex-row justify-between items-center gap-6 md:gap-0 relative">
                <div class="flex items-center justify-between w-full md:w-auto order-2 md:order-1">
                     <button 
                        ...
                        @click="prevChapter"
                        class="w-full md:w-auto justify-center px-5 py-2.5 md:px-6 md:py-3 bg-stone-100 text-stone-600 rounded-lg hover:bg-stone-200 font-medium transition-all flex items-center gap-2 text-sm md:text-base hover-sketch"
                    >
                ...
                <div class="flex items-center justify-end w-full md:w-auto order-3">
                    <button 
                        ...
                        @click="nextChapter"
                        class="w-full md:w-auto px-6 py-3 bg-stone-900 text-stone-50 rounded-lg hover:bg-stone-800 font-medium transition-all flex items-center justify-center gap-2 group shadow-lg shadow-stone-900/10 text-sm md:text-base hover-sketch"
                    >
                     ...
                     <button 
                        v-else
                        @click="finishStory"
                        class="w-full md:w-auto px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-900/20 text-sm md:text-base hover-sketch"
                    >
                        Finish Story
                    </button>
                </div>
